...
void SessionSetupListener::on_failure(const paho_mqtt::token& tok) {
  spdlog::warn("Operation failed. {}", tok.get_message_id());
}

void SessionSetupListener::on_success(const paho_mqtt::token& tok) {
...
void DisconnectionListener::on_failure(const paho_mqtt::token& /*tok*/) {
  spdlog::error("MQTT disconnection failed.");
}

void DisconnectionListener::on_success(const paho_mqtt::token& /*tok*/) {
